...
define(["/scripts/utils.js"], function (utils) {
  ...
  function navigationToggleHandler(element, options) {
    var bodyElement = document.getElementsByTagName('body')[0]; // Handle navigation states
  ...
  function navigationDrawerClose(options) {
    var navigationDrawers = document.getElementsByClassName(options.menuDropdownOpen),
  ...
  function navigationDrawer(options) {
    ...
    // });
    var $dropDownToggle = document.querySelectorAll(options.drawerToggle),
  ...
  function handleBackDropEvent(options, event, bodyElement) {
    event.stopPropagation(); // If the click happened inside the the container, bail
  ...
  function toggleNavigation(options) {
    var navBarToggle = Array.prototype.slice.call(document.querySelectorAll(options.navBarToggle)),
        bodyElement = document.getElementsByTagName('body')[0]; // Add navigation marker
  ...
  navBar.init = function (_options) {
    // Initialize here
    var options = utils.extendDefaultOptions(_defaults, _options); // navigationOffsetMarker(options);
