...
a25.navbar = function ($, undefined) {
  ...
  function toggleNavigation(_options) {
    var options = $.extend({}, _defaults, _options); // Navbar toggle
    ...
    $(options.navBarToggle).on("click", function (event) {
      ...
      $("." + options.containedDropdownClass).removeClass(options.containedDropdownClass);
    });
  }

  function menuDrawer(_options) {
    var options = $.extend({}, _defaults, _options); // Initialize drop down menu

    var $dropdownToggle = $(options.drawerToggle),
        $navTree = $(".app-nav--level-1"),
        $openClass = "app-nav__dropdown",
        $dropdownOpenClass = "app-nav__link--open",
        $containedDropdownClass = "app-nav__item--has-dropdown"; // $dropdownToggle.dropdown();

    $(document).on("click", function (e) {
      e.stopPropagation();
      var $ignoredElements = $('.js-nav-toggle, .js-drawer-close, .app-nav__tab--trigger, .app-nav__tab--action, [data-toggle="fly-out"], .app-nav__dropdown, .app-nav__item--has-children'); //check if the clicked area is dropDown or not
      ...
      if (!$(e.target).is($ignoredElements)) {
        ...
        $(options.navBar).removeClass(options.navBarOverlay).addClass(options.navBarHidden);
    ...
    $dropdownToggle.on("click", function (e) {
      ...
      if ($el.next().hasClass("app-nav--level-1")) {
        ...
        if ($el.hasClass($dropdownOpenClass)) {
          ...
          $el.not($el).parent().removeClass($containedDropdownClass);
